...
class RenderPipeline(object):
    ...
    def __call__(self, vertices, triangles, normal, bg, texture=None):
        ...
        # 2. rasterization, [0, 1]
        if texture is None:
            render_img, depth = rasterize(vertices, triangles, light, bg=bg)
            return render_img, depth
        else:
            texture *= light
            render_img, depth = rasterize(vertices, triangles, texture, bg=bg)
            return render_img, depth
